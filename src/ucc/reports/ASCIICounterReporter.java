/**
 * ASCIICounterReporter class added as part of implementing the functionality of -ascii switch
...
/**
 * ASCIICounterReporter class is derived from CounterReporter class. This class
 ...
 *
 */
public class ASCIICounterReporter extends CounterReporter
{
   /** Instantiate the Log4j2 logger for this class */
   private static final Logger logger = LogManager.getLogger(ASCIICounterReporter.class);

   /**
    * Default constructor to instantiate ASCIICounterReporter object
    * 
    * @param numLangs
    *           Total number of languages for report generation
    */
   public ASCIICounterReporter(int numLangs)
   {
      super(ASCII_EXTENSION, numLangs);
   }

   /**
    * Constructor to instantiate ASCIICounterReporter object with given
    * parameters
    *
    * @param outFilePrefix
    ...
    *           Total number of languages for report generation
    */
   public ASCIICounterReporter(String outFilePrefix, String outDir, int numLangs)
   {
   ...
   {
      String hdr = "Language                         |    Number    |    Physical    |    Logical   "
               + Constants.NEW_LINE_SEPARATOR
   ...
   {
      String hdr = "Math    Trig  Logarithm   Calculation     Cond.    Logical   Preproc.   Assignment   Pointer   ";
   ...
   {
      // String hdr = "RESULTS FOR " + lang + " FILES" +
      // Constants.NEW_LINE_SEPARATOR
      // +
      // "Total,Blank,Comments,,Compiler,Data,Exec.,Logical,Physical,File,Module"
      // + Constants.NEW_LINE_SEPARATOR
   ...
   {
      // String hdr = "RESULTS SUMMARY" + addlInfo +
      // Constants.NEW_LINE_SEPARATOR
      // + "Total,Blank,Comments,,Compiler,Data,Exec.,,File,SLOC" +
      // Constants.NEW_LINE_SEPARATOR
      // +
         ...
         for (int i = 0; i < NumLangs; i++)
         {
            if (CntrSummaryResults.get(i).NumCountedFiles > 0)
            {
               ...
               try
               {
                  ...
                  for (int j = 0; j < cntrResults.size(); j++)
                  {
                     if (cntrResults.get(j).LangProperty != null && cntrResults.get(j).LangProperty.name() == lang)
                     {
                        if (!cntrResults.get(j).IsDup && cntrResults.get(j).IsCounted && !cntrResults.get(j).HasEmbCode)
                        {
                           ...
                           row = String.format("%1$8s", (cntrResults.get(j).NumTotalLines)) + " "
                                    + String.format("%1$7s", (cntrResults.get(j).NumBlankLines)) + " |"
                                    + String.format("%1$8s", (cntrResults.get(j).NumWholeComments)) + " "
                                    + String.format("%1$8s", (cntrResults.get(j).NumEmbeddedComments)) + " |"
                                    + String.format("%1$8s", (cntrResults.get(j).NumCompilerDirectives)) + " "
                                    + String.format("%1$7s", (cntrResults.get(j).NumDataDeclLog)) + " "
                                    + String.format("%1$8s", (cntrResults.get(j).NumExecInstrLog)) + "  |"
                                    + String.format("%1$8s", (cntrResults.get(j).NumLSLOC)) + " "
                                    + String.format("%1$8s", (cntrResults.get(j).NumPSLOC)) + "  | "
                  ...
                  row = String.format("%1$8s", (CntrSummaryResults.get(i).NumTotalLines)) + " "
                           + String.format("%1$7s", (CntrSummaryResults.get(i).NumBlankLines)) + " |"
                           + String.format("%1$8s", (CntrSummaryResults.get(i).NumWholeCmnts)) + " "
                           + String.format("%1$8s", (CntrSummaryResults.get(i).NumEmbeddedCmnts)) + " |"
                           + String.format("%1$8s", (CntrSummaryResults.get(i).NumCompDirs)) + " "
                           + String.format("%1$8s", (CntrSummaryResults.get(i).NumDataDeclPhys)) + " "
                           + String.format("%1$8s", (CntrSummaryResults.get(i).NumExecInstrPhys)) + " |"
                           + String.format("%1$8s", (CntrSummaryResults.get(i).NumPSLOC)) + "  | CODE  Physical"
                           + Constants.NEW_LINE_SEPARATOR;
                  bufWriter.write(row);
                  ...
                  row = String.format("%1$8s", (CntrSummaryResults.get(i).NumTotalLines)) + " "
                           ...
                           + String.format("%1$8s", (CntrSummaryResults.get(i).NumEmbeddedCmnts)) + " |"
                           + String.format("%1$8s", (CntrSummaryResults.get(i).NumCompDirs)) + " "
                           + String.format("%1$8s", (CntrSummaryResults.get(i).NumDataDeclLog)) + " "
                           + String.format("%1$8s", (CntrSummaryResults.get(i).NumExecInstrLog)) + " |"
                           + String.format("%1$8s", (CntrSummaryResults.get(i).NumLSLOC)) + "  | CODE  Logical"
                           + Constants.NEW_LINE_SEPARATOR;
                  bufWriter.write(row);

                  // Write file access/successful count info
                  row = "Number of files successfully accessed........................."
                           + String.format("%1$5s", (CntrSummaryResults.get(i).NumCountedFiles)) + "  out of  "
                           + CntrSummaryResults.get(i).NumTotalFiles + Constants.NEW_LINE_SEPARATOR;
                  bufWriter.newLine();
                  bufWriter.write(row);
   ...
   protected void GenUnifiedCntrReport(ArrayList<UCCFile> cntrResults)
   {
      ...
      if (!cntrResults.isEmpty())
      {
         ...
         try
         {
            ...
            row = String.format("%1$8s", (totalLines)) + " " + String.format("%1$7s", (totalBlankLines)) + " |"
                     + String.format("%1$8s", (totalWholeCmnts)) + " " + String.format("%1$8s", (totalEmbeddedCmnts))
                     + " |" + String.format("%1$8s", (totalCompDirs)) + " "
                     + String.format("%1$8s", (totalDataDeclPhys)) + " " + String.format("%1$8s", (totalExecInstrPhys))
                     + " |" + String.format("%1$8s", (totalPSLOC)) + "  | CODE  Physical"
                     + Constants.NEW_LINE_SEPARATOR;
            bufWriter.write(row);

            row = String.format("%1$8s", (totalLines)) + " " + String.format("%1$7s", (totalBlankLines)) + " |"
                     + String.format("%1$8s", (totalWholeCmnts)) + " " + String.format("%1$8s", (totalEmbeddedCmnts))
                     + " |" + String.format("%1$8s", (totalCompDirs)) + " " + String.format("%1$8s", (totalDataDeclLog))
                     + " " + String.format("%1$8s", (totalExecInstrLog)) + " |" + String.format("%1$8s", (totalLSLOC))
            ...
            row = "Number of files successfully accessed........................."
                     + String.format("%1$5s", (numCountedFiles)) + "  out of  " + numTotalFiles
                     + Constants.NEW_LINE_SEPARATOR;
            bufWriter.newLine();
            bufWriter.write(row);

            // Write PLSOC to SLOC ratio
            row = "Ratio of Physical to Logical SLOC.........................."
                     + String.format("%.2f", ComputeRatio(totalPSLOC, totalLSLOC)) + Constants.NEW_LINE_SEPARATOR;
            bufWriter.newLine();
            bufWriter.write(row);
         ...
         try
         {
            ...
            for (int i = 0; i < CntrSummaryResults.size(); i++)
            {
               if (CntrSummaryResults.get(i).NumCountedFiles > 0)
               {
                  ...
                  row = String.format("%1$-30s", (LanguagePropertiesType.values()[i].toString())) + "   |"
                           + String.format("%1$10s", (CntrSummaryResults.get(i).NumCountedFiles)) + "    |"
                           + String.format("%1$10s", (CntrSummaryResults.get(i).NumPSLOC)) + "      |"
                           + String.format("%1$10s", (CntrSummaryResults.get(i).NumLSLOC))
                           + Constants.NEW_LINE_SEPARATOR;
                  bufWriter.write(row);
            ...
            row = String.format("%1$-30s", ("Total")) + "   |" + String.format("%1$10s", (totalFiles)) + "    |"
                     + String.format("%1$10s", (totalPSLOC)) + "      |" + String.format("%1$10s", (totalLSLOC))
                     + Constants.NEW_LINE_SEPARATOR;
            bufWriter.write(row);
   ...
   protected void GenUncountedReport(ArrayList<UCCFile> cntrResults)
   {
      ...
      if (!CntrSummaryResults.isEmpty() && UncountedFilesFound)
      {
         ...
         try
         {
            ...
            // Write column headers for data
            // bufWriter.write("Message,Uncounted File" +
            // Constants.NEW_LINE_SEPARATOR);

            String row;
            //
            row = String.format("%1$-45s", ("Message")) + "  |  " + String.format("%1$-45s", ("Uncounted File"))
                     + Constants.NEW_LINE_SEPARATOR;
            bufWriter.write(row);
            ...
            for (int i = 0; i < cntrResults.size(); i++)
            {
               ...
               {
                  // row = "Unknown File Extension," +
                  // cntrResults.get(i).FileName + Constants.NEW_LINE_SEPARATOR;
                  // bufWriter.write(row);

                  row = String.format("%1$-45s", ("Unknown File Extension")) + "  |  "
                           + String.format("%1$-45s", (cntrResults.get(i).FileName)) + Constants.NEW_LINE_SEPARATOR;
                  bufWriter.write(row);
               }
               // Report any files that were not counted by the code counter
               else if (!cntrResults.get(i).IsDup && !cntrResults.get(i).IsCounted)
               {
                  // row = "Error counting the file," +
                  // cntrResults.get(i).FileName + Constants.NEW_LINE_SEPARATOR;
                  // bufWriter.write(row);

                  row = String.format("%1$-45s", ("Error counting the file")) + "  |  "
                           + String.format("%1$-45s", (cntrResults.get(i).FileName)) + Constants.NEW_LINE_SEPARATOR;
                  bufWriter.write(row);
               }
            }
            ...
            if (cntrResSum.get(i).NumCountedFiles > 0)
            {
               ...
               try
               {
                           ...
                           || !cntrResSum.get(i).CmplxAssignCnts.isEmpty())
                  {
                     ...
                     // bufWriter.write("Math
                     // Functions,,Trigonometric,,Logarithmic,,Calculations,,Conditionals,,"
                     // + "Logical,,Preprocessor,,Assignment,,Pointer" +
                     // Constants.NEW_LINE_SEPARATOR);
                     bufWriter.write(
                              ...
                              || pntrIdx < cntrResSum.get(i).CmplxPntrCnts.size())
                     {
                        ...
                        if (mathIdx < cntrResSum.get(i).CmplxMathCnts.size())
                        {
                           row += cntrResSum.get(i).CmplxMathCnts.get(mathIdx).Keyword
                                    + dots.substring(cntrResSum.get(i).CmplxMathCnts.get(mathIdx).Keyword.length())
                                    + String.format("%1$4s", (cntrResSum.get(i).CmplxMathCnts.get(mathIdx).Count));
                           mathIdx++;
                        }
                        else
                        {
                        ...
                        if (trigIdx < cntrResSum.get(i).CmplxTrigCnts.size())
                        {
                           row += cntrResSum.get(i).CmplxTrigCnts.get(trigIdx).Keyword
                                    + dots.substring(cntrResSum.get(i).CmplxTrigCnts.get(trigIdx).Keyword.length())
                                    + String.format("%1$4s", (cntrResSum.get(i).CmplxTrigCnts.get(trigIdx).Count));
                           trigIdx++;
                        }
                        else
                        {
                        ...
                        if (logIdx < cntrResSum.get(i).CmplxLogCnts.size())
                        {
                           row += cntrResSum.get(i).CmplxLogCnts.get(logIdx).Keyword
                                    + dots.substring(cntrResSum.get(i).CmplxLogCnts.get(logIdx).Keyword.length())
                                    + String.format("%1$4s", (cntrResSum.get(i).CmplxLogCnts.get(logIdx).Count));
                           logIdx++;
                        }
                        else
                        {
                        ...
                        if (calcIdx < cntrResSum.get(i).CmplxCalcCnts.size())
                        {
                           row += cntrResSum.get(i).CmplxCalcCnts.get(calcIdx).Keyword
                                    + dots.substring(cntrResSum.get(i).CmplxCalcCnts.get(calcIdx).Keyword.length())
                                    + String.format("%1$4s", (cntrResSum.get(i).CmplxCalcCnts.get(calcIdx).Count));
                           calcIdx++;
                        }
                        else
                        {
                        ...
                        if (condIdx < cntrResSum.get(i).CmplxCondCnts.size())
                        {
                           row += cntrResSum.get(i).CmplxCondCnts.get(condIdx).Keyword
                                    + dots.substring(cntrResSum.get(i).CmplxCondCnts.get(condIdx).Keyword.length())
                                    + String.format("%1$4s", (cntrResSum.get(i).CmplxCondCnts.get(condIdx).Count));
                           condIdx++;
                        }
                        else
                        {
                        ...
                        if (logicIdx < cntrResSum.get(i).CmplxLogicCnts.size())
                        {
                           row += cntrResSum.get(i).CmplxLogicCnts.get(logicIdx).Keyword
                                    + dots.substring(cntrResSum.get(i).CmplxLogicCnts.get(logicIdx).Keyword.length())
                                    + String.format("%1$4s", (cntrResSum.get(i).CmplxLogicCnts.get(logicIdx).Count));
                           logicIdx++;
                        }
                        else
                        {
                        ...
                        if (preprocIdx < cntrResSum.get(i).CmplxPreprocCnts.size())
                        {
                           row += cntrResSum.get(i).CmplxPreprocCnts.get(preprocIdx).Keyword
                                    + dots.substring(
                                             cntrResSum.get(i).CmplxPreprocCnts.get(preprocIdx).Keyword.length())
                                    + String.format("%1$4s",
                        ...
                        if (assignIdx < cntrResSum.get(i).CmplxAssignCnts.size())
                        {
                           row += cntrResSum.get(i).CmplxAssignCnts.get(assignIdx).Keyword
                                    + dots.substring(cntrResSum.get(i).CmplxAssignCnts.get(assignIdx).Keyword.length())
                                    + String.format("%1$4s", (cntrResSum.get(i).CmplxAssignCnts.get(assignIdx).Count));
                           assignIdx++;
                        }
                        else
                        {
                        ...
                        if (pntrIdx < cntrResSum.get(i).CmplxPntrCnts.size())
                        {
                           row += cntrResSum.get(i).CmplxPntrCnts.get(pntrIdx).Keyword
                                    + dots.substring(cntrResSum.get(i).CmplxPntrCnts.get(pntrIdx).Keyword.length())
                                    + String.format("%1$4s", (cntrResSum.get(i).CmplxPntrCnts.get(pntrIdx).Count));
                           pntrIdx++;
                        }
                        else
                        {
         ...
         if (cntrResSum.get(i).NumCountedFiles > 0)
         {
            ...
            try
            {
                        ...
                        || execIdx < cntrResSum.get(i).ExecKeywrdCnts.size())
               {
                  ...
                  {
                     row += cntrResSum.get(i).CmplxPreprocCnts.get(preprocIdx).Keyword
                              + dots.substring(cntrResSum.get(i).CmplxPreprocCnts.get(preprocIdx).Keyword.length())
                              + String.format("%1$7s", (cntrResSum.get(i).CmplxPreprocCnts.get(preprocIdx).Count));
                     preprocIdx++;
                  }
                  else
                  {
                  ...
                  if (dataIdx < cntrResSum.get(i).DataKeywrdCnts.size())
                  {
                     row += cntrResSum.get(i).DataKeywrdCnts.get(dataIdx).Keyword
                              + dots.substring(cntrResSum.get(i).DataKeywrdCnts.get(dataIdx).Keyword.length())
                              + String.format("%1$7s", (cntrResSum.get(i).DataKeywrdCnts.get(dataIdx).Count));
                     dataIdx++;
                  }
                  else
                  {
                  ...
                  if (execIdx < cntrResSum.get(i).ExecKeywrdCnts.size())
                  {
                     row += cntrResSum.get(i).ExecKeywrdCnts.get(execIdx).Keyword
                              + dots.substring(cntrResSum.get(i).ExecKeywrdCnts.get(execIdx).Keyword.length())
                              + String.format("%1$7s", (cntrResSum.get(i).ExecKeywrdCnts.get(execIdx).Count));
                     execIdx++;
                  }
                  else
                  {
   ...
   protected void GenCmplxCntrReport()
   {
      ...
      if (!CmplxCntrSummaryResults.isEmpty())
      {
         ...
         try
         {
            ...
            {
               row = String.format("%1$5s", (CmplxCntrSummaryResults.get(i).NumMathOps))
                        + String.format("%1$8s", (CmplxCntrSummaryResults.get(i).NumTrigOps))
                        + String.format("%1$11s", (CmplxCntrSummaryResults.get(i).NumLogOps))
                        + String.format("%1$11s", (CmplxCntrSummaryResults.get(i).NumCalcOps))
                        + String.format("%1$11s", (CmplxCntrSummaryResults.get(i).NumCondStmts))
                        + String.format("%1$11s", (CmplxCntrSummaryResults.get(i).NumLogicOps))
                        + String.format("%1$11s", (CmplxCntrSummaryResults.get(i).NumPreprocs))
                        + String.format("%1$11s", (CmplxCntrSummaryResults.get(i).NumAssignOps))
                        + String.format("%1$11s", (CmplxCntrSummaryResults.get(i).NumPntrOps));

               for (int j = 0; j < maxLoopLvl; j++)
               {
                  if (j < CmplxCntrSummaryResults.get(i).LoopLvlCnts.size())
                  {
                     row += String.format("%1$11s", (CmplxCntrSummaryResults.get(i).LoopLvlCnts.get(j).toString()));
                  }
                  else
                  {
                     row += "          0";
            ...
            row = String.format("%1$5s", (totalMathOps)) + String.format("%1$8s", (totalTrigOps))
                     + String.format("%1$11s", (totalLogOps)) + String.format("%1$11s", (totalCalcOps))
                     + String.format("%1$11s", (totalCondStmts)) + String.format("%1$11s", (totalLogicOps))
                     + String.format("%1$11s", (totalPreprocs)) + String.format("%1$11s", (totalAssignOps))
                     + String.format("%1$11s", (totalPntrOps)) + Constants.NEW_LINE_SEPARATOR;
            bufWriter.write(row);
   ...
   private String GetCyclCmplxFileDataColHeader()
   {
      ...
      hdr += "Total(CC1) Total(CC2) Total(CC3) Average_CC1     Risk     |   Name" + Constants.NEW_LINE_SEPARATOR;
      hdr += "----------------------------------------------------------+-----------------------"
               + Constants.NEW_LINE_SEPARATOR;
   ...
   private String GetCyclCmplxFuncDataColHeader()
   {
      ...
      hdr += "Cyclomatic Complexity" + Constants.NEW_LINE_SEPARATOR;
      hdr += "       CC1          CC2          CC3   Risk        Function Name                                                  |   File Name"
               + Constants.NEW_LINE_SEPARATOR;
         ...
         try
         {
            ...
            for (int i = 0; i < cntrResults.size(); i++)
            {
                        ...
                        && !cntrResults.get(i).CyclCmplxCnts.isEmpty() && !cntrResults.get(i).HasEmbCode)
               {
                  ...
                  row = String.format("%1$10s", (cntrResults.get(i).CyclCmplxTotal)) // CC1
                                                                                     // total
                           + String.format("%1$10s", (" ")) // CC2 placeholder
                           + String.format("%1$10s", (" ")) + "  " + // CC3
                                                                     // placeholder
                           String.format("%1$10s", (String.format("%.2f", cntrResults.get(i).CyclCmplxAvg))) + "       "
                           + // CC1 average complexity
                           String.format("%1$-9s", (GetCyclCmplxRiskLvl(cntrResults.get(i).CyclCmplxAvg))) + "|   " // Risk
                                                                                                                    // level
                           + String.format("%1$-60s", (cntrResults.get(i).FileName)) + Constants.NEW_LINE_SEPARATOR; // File
                                                                                                                     // Name
                  bufWriter.write(row);
            ...
            // bufWriter.newLine();
            row = String.format("%1$10s", (totalCC1)) + String.format("%1$10s", (" ")) + String.format("%1$10s", (" "))
                     + String.format("%1$10s", (" ")) + String.format("%1$9s", (" "))
                     + String.format("%1$-9s", ("Totals")) + "|   " + numFuncs + " Functions in " + numFiles
                     + " File(s)" + Constants.NEW_LINE_SEPARATOR;

            row += String.format("%1$10s", (String.format("%.2f", avgCC1PerFunc))) + String.format("%1$10s", (" "))
                     + String.format("%1$10s", (" ")) + String.format("%1$10s", (" ")) + String.format("%1$9s", (" "))
                     + String.format("%1$-9s", ("Averages")) + "|   " + String.format("%.2f", avgFuncPerFile)
                     + " Functions per File (Averages = Totals/Functions)" + Constants.NEW_LINE_SEPARATOR;
            bufWriter.write(row);
            ...
            for (int i = 0; i < cntrResults.size(); i++)
            {
               if (!cntrResults.get(i).IsDup && cntrResults.get(i).IsCounted && !cntrResults.get(i).HasEmbCode)
               {
                  ...
                  {
                     row = String.format("%1$10s", (cntrResults.get(i).CyclCmplxCnts.get(funcIdx).Count))
                              + String.format("%1$10s", (" ")) + // CC1 total
                              String.format("%1$10s", (" ")) + // CC2
                                                               // placeholder
                              String.format("%1$9s", (" ")) + // CC3 placeholder
                              String.format("%1$-9s",
                                       (GetCyclCmplxRiskLvl(
                                                (double) cntrResults.get(i).CyclCmplxCnts.get(funcIdx).Count)))
                              + "    "
                              + String.format("%1$-60s", (cntrResults.get(i).CyclCmplxCnts.get(funcIdx).Keyword))
                              + "  |  " + // Function name
                              String.format("%1$-60s", (cntrResults.get(i).FileName + Constants.NEW_LINE_SEPARATOR)); // File
                                                                                                                      // Name
                     bufWriter.write(row);
            ...
            bufWriter.write(
                     "------------------------------------------------------------------------------------------------------------------+------------------------------------------"
                              + Constants.NEW_LINE_SEPARATOR);
            row = String.format("%1$10s", (totalCC1)) + String.format("%1$10s", (" ")) + String.format("%1$10s", (" "))
                     + String.format("%1$9s", (" ")) + String.format("%1$-10s", ("Totals")) + "   " + numFuncs
                     + " Functions " + numFiles + " File(s)" + Constants.NEW_LINE_SEPARATOR;

            row += String.format("%1$10s", (String.format("%.2f", avgCC1PerFunc))) + String.format("%1$10s", (" "))
                     + String.format("%1$10s", (" ")) + String.format("%1$9s", (" "))
                     + String.format("%1$-10s", ("Averages")) + "   " + String.format("%.2f", avgFuncPerFile)
                     + " Functions per File (Averages = Totals/Functions)" + Constants.NEW_LINE_SEPARATOR;
            bufWriter.write(row);
   ...
   protected void GenDupPairsReport(ArrayList<UCCFile> cntrResults)
   {
      ...
      if (!CntrSummaryResults.isEmpty())
      {
         ...
         try
         {
            ...
            // Write column headers for data
            // bufWriter.write("Original,Duplicate,Percent Duplicate" +
            // Constants.NEW_LINE_SEPARATOR);

            String row;

            row = String.format("%1$-80s", ("Original")) + "  |  " + String.format("%1$-80s", ("Duplicate")) + "  |  "
                     + String.format("%1$-45s", ("Percent Duplicate")) + Constants.NEW_LINE_SEPARATOR;
            ...
            for (int i = 0; i < cntrResults.size(); i++)
            {
               if (cntrResults.get(i).IsDup)
               {
                  ...
                  row = String.format("%1$-80s", (cntrResults.get(cntrResults.get(i).DupOfIdx).FileName)) + "  |  "
                           + String.format("%1$-80s", (cntrResults.get(i).FileName)) + "  |  "
                           + String.format("%1$-45s", (String.format("%.2f", cntrResults.get(i).DupCodePercent) + "%"))
                           + Constants.NEW_LINE_SEPARATOR;
                  bufWriter.write(row);
               }
            }
         ...
         try
         {
            ...
            for (int i = 0; i < DupCntrSummaryResults.size(); i++)
            {
               if (DupCntrSummaryResults.get(i).NumCountedFiles > 0)
               {
                  // Write summary of count data for each language
                  row = String.format("%1$-30s", (LanguagePropertiesType.values()[i].toString())) + "   |"
                           + String.format("%1$10s", (DupCntrSummaryResults.get(i).NumCountedFiles)) + "    |"
                           + String.format("%1$10s", (DupCntrSummaryResults.get(i).NumPSLOC)) + "      |"
                           + String.format("%1$10s", (DupCntrSummaryResults.get(i).NumLSLOC))
                           + Constants.NEW_LINE_SEPARATOR;
                  bufWriter.write(row);
            ...
            // Write totals to the report
            // row = "Total," + totalFiles + "," + totalPSLOC + "," + totalLSLOC
            // + Constants.NEW_LINE_SEPARATOR;
            /// bufWriter.write(row);
               ...
               try
               {
                  ...
                  for (int j = 0; j < cntrResults.size(); j++)
                  {
                     if (cntrResults.get(j).LangProperty != null && cntrResults.get(j).LangProperty.name() == lang)
                     {
                        ...
                        {
                           // Write count data for each file
                           row = String.format("%1$8s", (cntrResults.get(idx).NumTotalLines)) + " "
                                    + String.format("%1$7s", (cntrResults.get(idx).NumBlankLines)) + " |"
                                    + String.format("%1$8s", (cntrResults.get(idx).NumWholeComments)) + " "
                                    + String.format("%1$8s", (cntrResults.get(idx).NumEmbeddedComments)) + " |"
                                    + String.format("%1$8s", (cntrResults.get(idx).NumCompilerDirectives)) + " "
                                    + String.format("%1$7s", (cntrResults.get(idx).NumDataDeclLog)) + " "
                                    + String.format("%1$8s", (cntrResults.get(idx).NumExecInstrLog)) + "  |"
                                    + String.format("%1$8s", (cntrResults.get(idx).NumLSLOC)) + " "
                                    + String.format("%1$8s", (cntrResults.get(idx).NumPSLOC)) + "  | "
                  ...
                  // Write summary block data for the language
                  row = String.format("%1$8s", (DupCntrSummaryResults.get(i).NumTotalLines)) + " "
                           + String.format("%1$7s", (DupCntrSummaryResults.get(i).NumBlankLines)) + " |"
                           + String.format("%1$8s", (DupCntrSummaryResults.get(i).NumWholeCmnts)) + " "
                           + String.format("%1$8s", (DupCntrSummaryResults.get(i).NumEmbeddedCmnts)) + " |"
                           + String.format("%1$8s", (DupCntrSummaryResults.get(i).NumCompDirs)) + " "
                           + String.format("%1$8s", (DupCntrSummaryResults.get(i).NumDataDeclPhys)) + " "
                           + String.format("%1$8s", (DupCntrSummaryResults.get(i).NumExecInstrPhys)) + " |"
                           + String.format("%1$8s", (DupCntrSummaryResults.get(i).NumPSLOC)) + "  | CODE  Physical"
                           + Constants.NEW_LINE_SEPARATOR;
                  bufWriter.write(row);
                  ...
                  row = String.format("%1$8s", (DupCntrSummaryResults.get(i).NumTotalLines)) + " "
                           ...
                           + String.format("%1$8s", (DupCntrSummaryResults.get(i).NumEmbeddedCmnts)) + " |"
                           + String.format("%1$8s", (DupCntrSummaryResults.get(i).NumCompDirs)) + " "
                           + String.format("%1$8s", (DupCntrSummaryResults.get(i).NumDataDeclLog)) + " "
                           + String.format("%1$8s", (DupCntrSummaryResults.get(i).NumExecInstrLog)) + " |"
                           + String.format("%1$8s", (DupCntrSummaryResults.get(i).NumLSLOC)) + "  | CODE  Logical"
                           + Constants.NEW_LINE_SEPARATOR;
                  bufWriter.write(row);

                  row = "Number of files successfully accessed........................."
                           + String.format("%1$5s", (DupCntrSummaryResults.get(i).NumCountedFiles)) + "  out of  "
                           + DupCntrSummaryResults.get(i).NumTotalFiles + Constants.NEW_LINE_SEPARATOR;
                  bufWriter.newLine();
                  bufWriter.write(row);
   ...
   protected void GenDupCmplxCntrReport()
   {
      ...
      if (!DupCmplxCntrSummaryResults.isEmpty())
      {
         ...
         try
         {
            ...
            {
               row = String.format("%1$5s", (DupCmplxCntrSummaryResults.get(i).NumMathOps))
                        + String.format("%1$8s", (DupCmplxCntrSummaryResults.get(i).NumTrigOps))
                        + String.format("%1$11s", (DupCmplxCntrSummaryResults.get(i).NumLogOps))
                        + String.format("%1$11s", (DupCmplxCntrSummaryResults.get(i).NumCalcOps))
                        + String.format("%1$11s", (DupCmplxCntrSummaryResults.get(i).NumCondStmts))
                        + String.format("%1$11s", (DupCmplxCntrSummaryResults.get(i).NumLogicOps))
                        + String.format("%1$11s", (DupCmplxCntrSummaryResults.get(i).NumPreprocs))
                        + String.format("%1$12s", (DupCmplxCntrSummaryResults.get(i).NumAssignOps))
                        + String.format("%1$13s", (DupCmplxCntrSummaryResults.get(i).NumPntrOps));

               for (int j = 0; j < maxLoopLvl; j++)
               {
                  if (j < DupCmplxCntrSummaryResults.get(i).LoopLvlCnts.size())
                  {
                     row += String.format("%1$11s", (DupCmplxCntrSummaryResults.get(i).LoopLvlCnts.get(j).toString()));
                  }
                  else
                  {
                     row += "          0";
            ...
            row = String.format("%1$5s", (totalMathOps)) + String.format("%1$8s", (totalTrigOps))
                     + String.format("%1$11s", (totalLogOps)) + String.format("%1$11s", (totalCalcOps))
                     + String.format("%1$11s", (totalCondStmts)) + String.format("%1$11s", (totalLogicOps))
                     + String.format("%1$11s", (totalPreprocs)) + String.format("%1$12s", (totalAssignOps))
                     + String.format("%1$13s", (totalPntrOps)) + Constants.NEW_LINE_SEPARATOR;
            bufWriter.write(row);
      ...
      if (!DupCntrSummaryResults.isEmpty() && CyclomaticComplexityResFound)
      {
         ...
         try
         {
            ...
            for (int i = 0; i < cntrResults.size(); i++)
            {
               ...
               {
                  row = String.format("%1$10s", (cntrResults.get(idx).CyclCmplxTotal)) // CC1
                                                                                       // total
                           + String.format("%1$10s", (" ")) // CC2 placeholder
                           + String.format("%1$10s", (" ")) + "  " + // CC3
                                                                     // placeholder
                           String.format("%1$10s", (String.format("%.2f", cntrResults.get(idx).CyclCmplxAvg)))
                           + "       " + // CC1 average complexity
                           String.format("%1$-9s", (GetCyclCmplxRiskLvl(cntrResults.get(idx).CyclCmplxAvg))) + "|   " // Risk
                                                                                                                      // level
                           + String.format("%1$-60s", (cntrResults.get(i).FileName)) + Constants.NEW_LINE_SEPARATOR; // File
                                                                                                                     // Name
                  bufWriter.write(row);
            ...
            for (int i = 0; i < cntrResults.size(); i++)
            {
               ...
               if (cntrResults.get(i).IsDup && cntrResults.get(idx).IsCounted)
               {
                  ...
                  {
                     row = String.format("%1$10s", (cntrResults.get(idx).CyclCmplxCnts.get(funcIdx).Count))
                              + String.format("%1$10s", (" ")) + // CC1 total
                              String.format("%1$10s", (" ")) + // CC2
                                                               // placeholder
                              String.format("%1$9s", (" ")) + // CC3 placeholder
                              String.format("%1$-9s",
                                       (GetCyclCmplxRiskLvl(
                                                (double) cntrResults.get(idx).CyclCmplxCnts.get(funcIdx).Count)))
                              + "    "
                              + String.format("%1$-60s", (cntrResults.get(idx).CyclCmplxCnts.get(funcIdx).Keyword))
                              + "  |  " + // Function name
                              String.format("%1$-60s", (cntrResults.get(i).FileName)) + Constants.NEW_LINE_SEPARATOR; // File
                                                                                                                      // Name
                     bufWriter.write(row);
